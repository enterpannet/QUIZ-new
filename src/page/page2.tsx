...
export default function Page2() {
  return (
    <div className={pageShell}>
      <KioskSpringBandTop />
      <div
        className="relative isolate w-full overflow-hidden rounded-2xl bg-neutral-100 pb-4 max-xl:rounded-xl max-xl:pb-3 max-xl:md:min-h-[clamp(14rem,36vh,24rem)] max-xl:lg:min-h-[clamp(16rem,40vh,27rem)] max-xl:md:pb-7 max-xl:lg:pb-9 xl:sm:pb-5 "
      ...
      >
        ...
        />
        <div
          className="pointer-events-none absolute inset-0 z-50 flex max-h-full min-h-0 flex-col items-center justify-center gap-y-1 overflow-y-auto overscroll-contain px-3 py-2 text-center text-neutral-800 [text-shadow:0_1px_0_rgb(255_255_255/0.4),0_0_14px_rgb(245_245_245/0.9)] [scrollbar-width:thin] max-xl:gap-y-1 max-xl:px-3 max-xl:py-2 max-xl:sm:gap-y-1.5 max-xl:sm:px-4 max-xl:md:gap-y-1.5 max-xl:md:px-5 max-xl:md:py-3 max-xl:lg:gap-y-2 max-xl:lg:px-6 min-[1080px]:max-xl:gap-y-2 min-[1080px]:max-xl:px-6 xl:gap-y-1 xl:px-5 xl:sm:px-6"
        >
          <p className="mx-auto w-full max-w-[min(100%,100vw-1.75rem)] min-w-0 truncate font-bold leading-[1.08] tracking-tight text-[clamp(0.4375rem,1.5vmin+0.28rem,1.28rem)] min-[1080px]:leading-[1.06] min-[1080px]:text-[clamp(0.5625rem,1.68vmax+0.42rem,3rem)] xl:leading-[1.06] xl:text-[clamp(0.5625rem,1.72vmax+0.44rem,3.1875rem)] 2xl:leading-[1.1] 2xl:text-[clamp(0.5625rem,1.92vmax+0.48rem,3.375rem)]">{middleFrameLines[0]}</p>
          <p className="mx-auto w-full max-w-[min(100%,100vw-1.75rem)] min-w-0 truncate font-bold leading-snug text-[clamp(0.4375rem,1vmin+0.34rem,1.1rem)] min-[1080px]:text-[clamp(0.5625rem,1.2vmax+0.48rem,2.375rem)] xl:text-[clamp(0.5625rem,1.24vmax+0.5rem,2.5rem)] 2xl:text-[clamp(0.5625rem,1.32vmax+0.5rem,2.75rem)]">{middleFrameLines[1]}</p>
          <p className="mx-auto w-full max-w-[min(100%,42rem)] text-balance break-words font-semibold leading-snug text-[clamp(0.53125rem,0.82vmin+0.36rem,0.98rem)] min-[1080px]:text-[clamp(0.6875rem,0.9vmax+0.52rem,2rem)] xl:text-[clamp(0.6875rem,0.94vmax+0.54rem,2.125rem)] 2xl:text-[clamp(0.6875rem,0.93vmax+0.54rem,2.125rem)]">{middleFrameLines[2]}</p>
